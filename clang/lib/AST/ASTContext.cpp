...
CreateX86_64ABIBuiltinVaListDecl(const ASTContext *Context) {
  ...
  const size_t NumFields = 4 + 1; // AWC CHANGE - Increase number of fields in va_list by 1
  QualType FieldTypes[NumFields];
  const char *FieldNames[NumFields];
  ...
  // AWC CHANGE - Add remaining to va_list
  FieldTypes[4] = Context->getSignedSizeType();;
  FieldNames[4] = "remaining";
  // AWC CHANGE END
